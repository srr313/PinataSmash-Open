...
static NSString* KCrystalLeaderboardTypeUpdated = @"CCLeaderboardTypeUpdated";
...
@interface AppController (Crystal) <CrystalSessionDelegate, CrystalPlayerDelegate, CrystalVirtualGoodsDelegate, CrystalLeaderboardDelegate, CrystalAchievementDelegate>
...
- (void) cmdResetCrystalPullTabState;

- (void) cmdActivatePullTabUiWithType:(NSString*)type fromScreenEdge:(NSString*)edgeString closed:(NSString*)closedState;
- (void) cmdActivatePullTabUiAtLeaderboardWithId:(NSString*)leaderboardId fromScreenEdge:(NSString*)screenEdge closed:(NSString*)closedState;
- (void) cmdActivateCrystalPullTabOn:(NSString*)tabStrings fromScreenEdge:(NSString*)edgeString closed:(NSString*)closedState;
...
- (void) cmdPostLeaderboardResultWithId:(NSString*)idString result:(NSString*)result lowestValFirst:(NSString*)lowestValFirst forGameCenterId:(NSString*)gcLeaderboardId isTimeBased:(NSString*)isTimeBased;
...
- (void) cmdDownloadLeaderboardDataForID:(NSString*)leaderboardID leaderboardType:(NSString*)leaderboardType force:(NSString*)force;
...
- (void) cmdRequestAchievementsData;
...
- (void) applicationDidFinishLaunching:(UIApplication*)application
{
	...
	[CrystalAchievements sharedInstance].delegate = self;
...
{
	// Set a key that will be picked up in the c#
	NSLog(@"[Crystal] crystalLeaderboardUpdated %@ type %d", leaderboardId, leaderboardType);
	
	CrystalLeaderboards* leaderBoard = [CrystalLeaderboards sharedInstance];
	
	NSArray* top20EntriesForLeaderboardID = [leaderBoard top20EntriesForLeaderboardID:leaderboardId leaderboardType:leaderboardType];
	NSArray* top20FriendsForLeaderboardID = [leaderBoard top20FriendsForLeaderboardID:leaderboardId leaderboardType:leaderboardType];
	NSArray* random20ForLeaderboardID = [leaderBoard random20ForLeaderboardID:leaderboardId leaderboardType:leaderboardType];
	NSDictionary* currentUserEntryForLeaderboardID = [leaderBoard currentUserEntryForLeaderboardID:leaderboardId leaderboardType:leaderboardType];	
	...
	[[NSUserDefaults standardUserDefaults] setObject:[NSString stringWithFormat:@"%d", leaderboardType] forKey:KCrystalLeaderboardTypeUpdated];
...
#pragma mark -
#pragma mark CrystalAchievementDelegate

- (void) crystalAchievementsUpdated
{
	// Set a key that will be picked up in the c#
	NSLog(@"[Crystal] crystalAchievementsUpdated");
	CrystalAchievements* achievements = [CrystalAchievements sharedInstance];
	
	NSString* achievementsDataStr = [CrystalSession convertToNSString:[achievements getAchievementsUnlocked]];
	
	if (achievementsDataStr)
	{
		[[NSUserDefaults standardUserDefaults] setObject:achievementsDataStr forKey:KCrystalAchievementsUpdated];
	}
	else
	{
		[[NSUserDefaults standardUserDefaults] setObject:@"" forKey:KCrystalAchievementsUpdated];
	}	
}
...
{
	NSLog(@"[Crystal]handleOpenURL [URL] Open: %@", url);
	
	[CrystalSession application:application handleOpenUrl:url];
...
- (void) updateStartedFromChallenge
{
	if ([CrystalSession appWasStartedFromPendingChallenge])
	{
		...
		_appWasStartedFromChallenge = YES;
	}
	else
	{
		if ( _appWasStartedFromChallenge )
		{
			_appWasStartedFromChallenge = NO;
...
- (void) processCommandChannels:(NSTimer*)theTimer
{
	...
	for (NSString* command in commands)
	{
		...
		else if (commandParam0 && ([commandParam0 compare:@"DeactivateUi"] == NSOrderedSame))
		{
			NSAssert(count == 1, @"[Crystal] require 0 param on ActivateUi");
			[self cmdDeactivateUi];
		}
		
		else if (commandParam0 && ([commandParam0 compare:@"ActivatePullTabUi"] == NSOrderedSame))
		{
			NSAssert(count == 4, @"[Crystal] require 3 params on ActivatePulltabUi");
			[self cmdActivatePullTabUiWithType:commandParam1 fromScreenEdge:commandParam2 closed:commandParam3];
		}
		
		else if( commandParam0 && ([commandParam0 compare:@"ActivatePullTabUiAtLeadboardWithId"] == NSOrderedSame))
		{
			NSAssert(count == 4, @"[Crystal] require 3 params on ActivatePullTabUiAtLeaderboardId");
			[self cmdActivatePullTabUiAtLeaderboardWithId:commandParam1 fromScreenEdge:commandParam2 closed:commandParam3];			
		}
		
		else if( commandParam0 && ([commandParam0 compare:@"ActivateCrystalPullTabOn"] == NSOrderedSame))
		{
			NSAssert(count == 4, @"[Crystal] require 3 params on ActivateCrystalPullTabOn");
			[self cmdActivateCrystalPullTabOn:commandParam1 fromScreenEdge:commandParam2 closed:commandParam3];			
		}
		...
		if (commandParam0 && ([commandParam0 compare:@"ResetCrystalPullTabState"] == NSOrderedSame))
		{
			NSAssert(count == 1, @"[Crystal] require 0 param on ResetCrystalPullTabState");
			[self cmdResetCrystalPullTabState];
		}
		...
		{
			NSAssert(count == 6, @"[Crystal] require 5 params on PostLeaderboardResult");
			[self cmdPostLeaderboardResultWithId:commandParam1 result:commandParam2 lowestValFirst:commandParam3 forGameCenterId:commandParam4 isTimeBased:commandParam5];
		}
		...
		{
			NSAssert(count == 4, @"[Crystal] require 3 params on DownloadLeaderboardDataForID");
			[self cmdDownloadLeaderboardDataForID:commandParam1 leaderboardType:commandParam2 force:commandParam3];
		}
		...
		else if (commandParam0 && ([commandParam0 compare:@"RequestAchievementData"] == NSOrderedSame))
		{
			NSAssert(count == 1, @"[Crystal] require 0 param on RequestAchievementData");
			[self cmdRequestAchievementsData];
		}
	}
...
- (void) cmdActivateUiWithType:(NSString*)type
{
	...
	switch (uiType) {
		...
		case GiftsAndMarket:
			[CrystalSession activateCrystalUIAtGiftsAndMarket];
			break;
		...
		case FindFriends:
			[CrystalSession activateCrystalUIAtFindFriends];
			break;
			
		case InviteFriends:
			[CrystalSession activateCrystalUIAtInviteFriends];
			break;
...
{
	[self cmdActivatePullTabUiWithType:type fromScreenEdge:edgeString closed:@"1"];
}
...
- (void) cmdActivatePullTabUiWithType:(NSString*)type fromScreenEdge:(NSString*)edgeString closed:(NSString*)closedState
{
	...
	BOOL closed = [closedState boolValue];
	...
	switch (uiType) {
		...
		case ProfileTab:
			[CrystalSession activateCrystalPullTabOnProfileFromScreenEdge:edgeString closed:closed];
			break;
			
		case ChallengesTab:
			[CrystalSession activateCrystalPullTabOnChallengesFromScreenEdge:edgeString closed:closed];
			break;
			
		case LeaderboardsTab:
			[CrystalSession activateCrystalPullTabOnLeaderboardsFromScreenEdge:edgeString closed:closed];
			break;
			
		case AchievementsTab:
			[CrystalSession activateCrystalPullTabOnAchievementsFromScreenEdge:edgeString closed:closed];
			break;
			
		case FriendsTab:
			[CrystalSession activateCrystalPullTabOnFriendsFromScreenEdge:edgeString closed:closed];
			break;
			
		case SettingsTab:
			[CrystalSession activateCrystalPullTabOnSettingsFromScreenEdge:edgeString closed:closed];
			break;
			
		case GiftingTab:
			[CrystalSession activateCrystalPullTabOnGiftsFromScreenEdge:edgeString closed:closed];
			break;
			
		case VirtualGoodsTab:
			[CrystalSession activateCrystalPullTabOnVirtualGoodsFromScreenEdge:edgeString closed:closed];
			break;
			
		case VirtualCurrenciesTab:
			[CrystalSession activateCrystalPullTabOnVirtualCurrenciesFromScreenEdge:edgeString closed:closed];
			break;
			
		case FindFriendsTab:
			[CrystalSession activateCrystalPullTabOnFindFriendsFromScreenEdge:edgeString closed:closed];
			break;
			
		case GiftsAndMarketTab:
			[CrystalSession activateCrystalPullTabOnGiftsAndMarketFromScreenEdge:edgeString closed:closed];
			break;
			
		case NewsTab:
			[CrystalSession activateCrystalPullTabOnNewsFromScreenEdge:edgeString closed:closed];
			break;
...
- (void) cmdActivatePullTabUiAtLeaderboardWithId:(NSString*)leaderboardId fromScreenEdge:(NSString*)edgeString closed:(NSString*)closedState
{
	NSLog(@"[Crystal] cmdActivatePullTabUiAtLeaderboardWithId");
	BOOL closed = [closedState boolValue];	
	[CrystalSession activateCrystalPullTabOnLeaderboardWithId:leaderboardId fromScreenEdge:edgeString closed:closed];
}

- (void) cmdActivateCrystalPullTabOn:(NSString*)tabStrings fromScreenEdge:(NSString*)edgeString closed:(NSString*)closedState
{
	NSLog(@"[Crystal] activateCrystalPullTabOn");
	BOOL closed = [closedState boolValue];	
	NSArray* tabStringsArray = nil;
	tabStringsArray = [CrystalSession convertDataStringToObject:tabStrings];
	[CrystalSession activateCrystalPullTabOn:tabStringsArray fromScreenEdge:edgeString closed:closed];
}
...
- (void) cmdResetCrystalPullTabState
{
	NSLog(@"[Crystal] resetCrystalPullTabState");
	[CrystalSession resetCrystalPullTabState];
}
...
- (void) cmdPostLeaderboardResultWithId:(NSString*)idString result:(NSString*)result lowestValFirst:(NSString*)lowestValFirst forGameCenterId:(NSString*)gcLeaderboardId isTimeBased:(NSString*)isTimeBased
{
	...
	BOOL timeBased = [isTimeBased boolValue];
	NSLog(@"[Crystal] PostLeaderboardResultWithId %@ result %f lowestvalue %d Game Center ID %@", idString, resultFloat, lowestValFirst, gcLeaderboardId, timeBased);
	
	if (_gameCenterEnabled)
		[CrystalSession postLeaderboardResult:resultFloat forLeaderboardId:idString lowestValFirst:lowestValBool forGameCenterLeaderboardId:gcLeaderboardId isTimeBased:timeBased];
	else
		[CrystalSession postLeaderboardResult:resultFloat forLeaderboardId:idString lowestValFirst:lowestValBool isTimeBased:timeBased];
}
...
- (void) cmdDownloadLeaderboardDataForID:(NSString*)leaderboardID leaderboardType:(NSString*)leaderboardType force:(NSString*)force
{
	CrystalLeaderboardType type = (CrystalLeaderboardType)[leaderboardType intValue];
	bool forceBool = [force boolValue];
	
	NSLog(@"[Crystal] DownloadLeaderboardDataForID: [leaderboardID %@] [CrystalLeaderboardType %d] force %d", leaderboardID, type, forceBool);
	
	if (leaderboardID != nil)
	{
		[[CrystalLeaderboards sharedInstance] downloadLeaderboardDataForID:leaderboardID leaderboardType:type force:forceBool];
	}
}
...
- (void) cmdRequestAchievementsData
{
	NSLog(@"[Crystal] CrystalAchievements getAchievementsUnlocked");
	//	Just kick off data request here and set result in the crystalAchievementsUpdated method
	[[CrystalAchievements sharedInstance] getAchievementsUnlocked];
}
...
- (void) clearCommandChannels
{
	...
	[[NSUserDefaults standardUserDefaults] removeObjectForKey:KCrystalLeaderboardTypeUpdated];
	...
	[[NSUserDefaults standardUserDefaults] removeObjectForKey:KCrystalAchievementsUpdated];
}
